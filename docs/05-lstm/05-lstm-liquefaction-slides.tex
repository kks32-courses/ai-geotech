...
\begin{equation}
\frac{\partial L}{\partial W} = \sum_{t=1}^{T} \frac{\partial L}{\partial h_T}
\left( \prod_{k=t+1}^{T} \frac{\partial h_k}{\partial h_{k-1}} \right)
\frac{\partial h_t}{\partial W}
\end{equation}

If $\left|\frac{\partial h_k}{\partial h_{k-1}}\right| < 1$, the inner product decays as
$T - t$ grows, so the early steps contribute nothing to the gradient

\vspace{1em}

\textbf{Impact:} Can't remember events 10+ steps ago

\textbf{For liquefaction:} the memory horizon here is one window, 800 steps at
0.013\,s, that is 10.4\,s. The LSTM state resets at every window.
...
\begin{enumerate}
\item Normalized time: $T_{\text{norm}} = t/600\,\text{s}$, a fixed constant, so no
trial leaks its total duration
\item Normalized shear stress: $\tau_{\text{norm}} = \tau/\sigma'_0$
\item Relative density: $D_r$ (constant)
\end{enumerate}

\textbf{Output:} pore pressure ratio $r_u \in [0, 1]$, predicted through a tanh unit
whose range is $(-1, 1)$, so the model can go negative and never reaches $r_u = 1$
exactly
...
\textbf{Training Set:}
\begin{itemize}
\item 14 trials: 4 from Exp 8, 10 from Exp 9
\item 70,631 windows at stride 4
\item Validation: 2 whole trials held out before windowing
\end{itemize}
...
\item Normalize time by a fixed 600\,s, stress by $\sigma'_0$, $D_r$ by 100
\end{itemize}
...
\item Validation is two whole held-out trials, so the gap is a real generalisation gap
...
\begin{columns}[T]
\column{0.5\textwidth}
\textbf{Test Set Metrics:} pooled over all five held-out trials, 50{,}687 windows

\begin{table}
\centering
\begin{tabular}{lccc}
\toprule
 & \textbf{Pooled} & \textbf{Exp 8} & \textbf{Exp 9} \\
\midrule
MSE & 0.116 & 0.029--0.037 & 0.041--0.310 \\
MAE & 0.268 & 0.123--0.158 & 0.184--0.498 \\
$R^2$ & 0.06 & 0.69--0.72 & $-2.32$ to $-1.22$ \\
\bottomrule
\end{tabular}
\end{table}

\footnotesize From a 12-epoch run. The model tracks the Experiment-8 modulated-sine
trials and fails on the Experiment-9 transient trials. Twelve epochs underfits.
\normalsize

\mode<beamer>{
\textbf{What LSTM Captures:}
\begin{itemize}
\item \checkmark  Shielding effect, on the Experiment-8 trials
\item \checkmark  Pore pressure buildup rate, on the Experiment-8 trials
\item $\times$ Experiment-9 transient loading, not yet
\end{itemize}
}
...
\textbf{Code:} \texttt{docs/05-lstm/05a-lstm-liquefaction.ipynb}

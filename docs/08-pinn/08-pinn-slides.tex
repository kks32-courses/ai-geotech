...
\vspace{1cm}
\centering
\href{https://colab.research.google.com/github/kks32-courses/ai-geotech/blob/main/docs/08-pinn/08a-pinn.ipynb}{\beamergotobutton{Open Notebook: PINN}}
...
AD gives the \textbf{exact derivative of the network}, to machine precision, by applying the chain rule through the computational graph. The network is still an approximation of the true solution, so the PINN still carries approximation and optimisation error.
\begin{itemize}
    \item No finite difference stencil and no truncation error in the derivative.
    \item Reverse mode is the cheap way to get the gradient with respect to the parameters; for the derivative of one output with respect to one input, forward mode costs less.
...
\begin{figure}[ht]
	...
	\caption*{Left: displacement against time. Both networks pass through the ten training points, which stop at $t = 0.36$; beyond them the standard network flattens out while the PINN tracks the true solution. Right: absolute error on a log scale.}
\end{figure}
...
\end{equation*}
The exact solution is $u(x) = \frac{1}{\pi}\sin(\pi x)$: the coefficient $\pi$ in the source term, rather than $\pi^2$, sets the amplitude to $1/\pi \approx 0.32$, the value on the comparison plot later.
...
\begin{frame}
	...
	\begin{figure}[ht]
		\centering
		\includegraphics[width=0.75\textwidth]{figs/inverse-heat-conductivity.png}
	\end{figure}
...
\end{equation*}
Here, the thermal conductivity $k$ is \textbf{unknown}. The diffusivity $\alpha = k/(\rho c_p)$ belongs to the transient equation, not to this one.
...
\begin{figure}[ht]
	\centering
	\includegraphics[width=\linewidth]{figs/inverse-heat-conductivity.png}

...
\textbf{Analysis:}
\begin{enumerate}
\item \textbf{GWD (1.8\%):} Valid extreme water table depths $\to$ Keep
\item \textbf{L (0.1\%):} Very few unusual distances $\to$ Keep
\item \textbf{Slope (7.4\%):} Natural field variability $\to$ Keep
\item \textbf{PGA (0.0\%):} Perfect - earthquake data well-controlled $\to$ Keep
\end{enumerate}

\vspace{0.5em}

\textbf{Decision:} All outliers $<$ 10\% and physically plausible $\to$ \textbf{Keep all data}
...
\begin{tabular}{lll}
\toprule
\textbf{Method} & \textbf{Use For} & \textbf{Notes} \\
\midrule
...
\textbf{Key Insight:} \\
\begin{itemize}
\item \textbf{Slope is NOT significantly different} $\to$ May not help prediction!
\item \textbf{GWD, L, PGA} show extremely low p-values $\to$ Key predictors
\end{itemize}
